...
2. Copy the Connector YML file to the new instance directory (instance1 in this example):
    - Run the following command:
      ```
      cp ~/privacera/privacera-manager/config/sample-vars/vars.connector.databricks.sql.analytics.yml .
      ```
...
1. Create a folder named `custom` in the directory of connector-env folder.
    - For instance1:
      ```
      mkdir ~/privacera/privacera-manager/config/custom-vars/connectors/databricks-sql-analytics/instance1/custom
      ```
...
2. Create a file named `connector-custom.properties` under the custom directory:
    ...
    - Run the following command:
      ```
      vi ~/privacera/privacera-manager/config/custom-vars/connectors/databricks-sql-analytics/instance1/custom/connector-custom.properties
      ```
...
3. Create a file named `connector-env-custom.sh` under the custom directory:
    ...
    - Run the following command:
      ```
      vi ~/privacera/privacera-manager/config/custom-vars/connectors/databricks-sql-analytics/instance1/custom/connector-env-custom.sh
      ```
